...
func TestWorkpodE2E(t *testing.T) {
	...
	// Test 5: Start code-serve-web and verify the token is printed to stdout
	...
	t.Run("CodeServeWeb", func(t *testing.T) {
		waitForPodRunning(t, client, testNamespace, "app=work-pod", 60*time.Second)

		output, err := runCommand(t, fullBinaryPath, "-config", fullConfigPath, "work-pod", "code-serve-web")
		if err != nil {
		...
		if !strings.Contains(output, "https://workpod.e2e-test.local/?tkn=") {
	...
	// Test 6: code-serve-web-token reads back the previously persisted token.
	t.Run("CodeServeWebToken", func(t *testing.T) {
		waitForPodRunning(t, client, testNamespace, "app=work-pod", 60*time.Second)

		output, err := runCommand(t, fullBinaryPath, "-config", fullConfigPath, "work-pod", "code-serve-web-token")
		if err != nil {
	...
	// Test 7: Clean up workpod resources
